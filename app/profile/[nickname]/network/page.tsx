...
export default function ProfileNetworkPage() {
  ...
  useEffect(() => {
    const cargarNetwork = async () => {
      ...
      try {
        ...
        if (followersData) {
          setFollowers(followersData.map((f: any) => Array.isArray(f.profiles) ? f.profiles[0] : f.profiles));
        }
        
        if (followingData) {
          setFollowing(followingData.map((f: any) => Array.isArray(f.profiles) ? f.profiles[0] : f.profiles));
        }
      } catch (error) {
        console.error(error);
      } finally {
  ...
  const handleFollow = async (targetId: string) => {
    ...
    try {
      ...
      if (!error) {
        const targetUser = followers.find((f) => f.id === targetId);
        if (targetUser) {
  ...
  if (loading) {
    return <div style={{ padding: "20px", textAlign: "center" }}>Cargando network...</div>;
  }
      ...
      <fieldset style={{ flex: 1, padding: "20px", backgroundColor: "#fff", minHeight: "300px" }}>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: "20px" }}>
          {followers.length > 0 ? (
            followers.map((user) => {
              ...
              return (
                <div key={user.id} style={{ display: "flex", alignItems: "center", gap: "15px" }}>
                  <div style={{ 
                    width: "55px", height: "55px", flexShrink: 0,
                  ...
                  <div style={{ display: "flex", flexDirection: "column", gap: "6px" }}>
                    ...
                    {!isFollowingBack && (
                      <button 
                        ...
                        style={{ padding: "2px 10px", fontSize: "12px", width: "fit-content" }}
                      >
                        Follow
                      </button>
                    )}
            ...
            <p style={{ color: "#666" }}>Aún no tienes seguidores.</p>
          )}
        </div>
      </fieldset>
      ...
      <fieldset style={{ flex: 1, padding: "20px", backgroundColor: "#fff", minHeight: "300px" }}>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: "20px" }}>
          {following.length > 0 ? (
            ...
            <p style={{ color: "#666" }}>No sigues a nadie todavía.</p>
          )}
        </div>
      </fieldset>
